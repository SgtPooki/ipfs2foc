...
test('matchPersona picks laptop tester for a tiny job', () => {
  const p = matchPersona({ totalAssets: 500, estimatedTotalBytes: 2 * GIB, carStoreFreeBytes: 40 * GIB, bandwidthMbit: 500 })
...
test('matchPersona picks production migrator with disk and scale', () => {
  const p = matchPersona({ totalAssets: 500_000, estimatedTotalBytes: 200 * GIB, carStoreFreeBytes: 300 * GIB, bandwidthMbit: 500 })
...
test('matchPersona picks bandwidth-bound when upload is slow', () => {
  const p = matchPersona({ totalAssets: 1_000_000, estimatedTotalBytes: 50 * 1024 * GIB, carStoreFreeBytes: 60 * GIB, bandwidthMbit: 20 })
  assert.equal(p.name, 'Bandwidth-bound migrator')
  assert.equal(p.flags.maxInFlight, 1)
})

test('matchPersona falls back to SMB when nothing else fits', () => {
  const p = matchPersona({ totalAssets: 50_000, estimatedTotalBytes: 30 * GIB, carStoreFreeBytes: 100 * GIB, bandwidthMbit: 200 })
  assert.equal(p.name, 'SMB / small studio')
})

test('applyCacheHeuristic forces cache when probe failure rate is high', () => {
  const base = matchPersona({ totalAssets: 50_000, estimatedTotalBytes: 30 * GIB, carStoreFreeBytes: 200 * GIB, bandwidthMbit: 200 })
...
test('applyCacheHeuristic downgrades to stream-only when disk is tight', () => {
  const base = matchPersona({ totalAssets: 50_000, estimatedTotalBytes: 30 * GIB, carStoreFreeBytes: 20 * GIB, bandwidthMbit: 200 })
